...
export class CheckoutService {
  ...
  private isCartReady: boolean;
  constructor() { this.isCartReady = false; }// Initial value }

  // Method to process the checkout (mocked payment processing)
  public processCheckout(cart: Array<any>, userDetails: any): string {
    ...
    const orderConfirmation = this.saveOrder(cart, userDetails);
    
    return orderConfirmation;  // Mocked confirmation message
  }

  // Save the order and return confirmation
  private saveOrder(cart: Array<any>, userDetails: any): string {
